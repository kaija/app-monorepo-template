...
class EnvValidator:
        ...
        if db_url:
            ...
            if os.getenv("ENVIRONMENT") == "production":
                if "postgres:postgres@" in db_url:
                    self.errors.append(
                        "DATABASE_URL contains default PostgreSQL credentials in production"
                    )
        ...
        if jwt_secret:
            ...
            if os.getenv("ENVIRONMENT") == "production":
                if "dev" in jwt_secret.lower() or "test" in jwt_secret.lower():
                    self.errors.append(
                        "JWT_SECRET_KEY appears to be a development key in production"
                    )
    ...
    def validate_nextauth_secret(self) -> None:
        ...
        if nextauth_secret:
            ...
            if os.getenv("ENVIRONMENT") == "production":
                if (
                    "dev" in nextauth_secret.lower()
                    or "test" in nextauth_secret.lower()
                ):
                    self.errors.append(
                        "NEXTAUTH_SECRET appears to be a development key in production"
                    )
            ...
            if url:
                ...
                if environment == "production":
                    if not url.startswith("https://") and "localhost" not in url:
                        self.errors.append(
                            f"{description} should use HTTPS in production: {url}"
                        )
    ...
    def validate_oauth_config(self) -> None:
        ...
        if apple_id and not apple_secret:
            self.warnings.append(
                "APPLE_CLIENT_ID is set but APPLE_CLIENT_SECRET is missing"
            )
        if apple_secret and not apple_id:
            self.warnings.append(
                "APPLE_CLIENT_SECRET is set but APPLE_CLIENT_ID is missing"
            )
        ...
        for var_name, var_value in [
            ...
            if var_value and var_value.lower() in [
                p.lower() for p in placeholder_values
            ]:
                        ...
                        with open(file_path, "r", encoding="utf-8") as f:
                            ...
                            for pattern, description in secret_patterns:
                                ...
                                for match in matches:
                                    ...
                                    self.warnings.append(
                                        f"{description} found in {file_path}:{line_num}"
                                    )
    ...
    def validate_all(self) -> Tuple[bool, List[str], List[str]]:
        ...
        if environment == "production":
            required_vars.extend(["POSTGRES_PASSWORD", "CORS_ORIGINS"])
...
def main():
    ...
    parser.add_argument(
        "--environment",
        help="Environment to validate for (development, staging, production)",
    )
