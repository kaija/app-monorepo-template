...
class Settings(BaseSettings):
    ...
    def validate_jwt_secret_key(cls, v: str) -> str:
        ...
        if v.lower() in [s.lower() for s in weak_secrets]:
            raise ValueError('JWT secret key appears to be a default/weak value. Use a cryptographically secure secret.')
    ...
    def oauth_enabled(self) -> bool:
        ...
        return bool(
            ...
            (self.apple_client_id and self.apple_client_secret)
        )
...
def create_settings() -> Settings:
    ...
    try:
        ...
        if settings.is_production():
            ...
            if 'dev' in settings.jwt_secret_key.lower() or 'test' in settings.jwt_secret_key.lower():
                production_checks.append("JWT_SECRET_KEY appears to be a development key")
            
            # Check for HTTPS in production CORS origins
            for origin in settings.cors_origins:
                if not origin.startswith('https://') and not origin.startswith('http://localhost'):
                    production_checks.append(f"CORS origin should use HTTPS in production: {origin}")
